 ...
 * cannot complete: the cached PKCE state is missing/expired/already used, or
 * the newly-authorized Claude account's email does not match the org
 * account being connected.
 *
 * Carries a machine-readable `reason` (e.g. 'connect_state_expired',
 * 'connect_email_mismatch') so the Filament action can show a friendly,
 * reason-specific notification. Per token hygiene requirements, callers
 * MUST NOT embed raw token material in the message.
 */
class AccountConnectException extends Exception
{
    /**
     * Machine-readable failure kind (e.g. 'connect_state_expired',
     * 'connect_email_mismatch') for callers to branch on without parsing
     * the message.
     *
     * @var string
     */
     ...
     *
     * @param  string  $reason  machine-readable failure kind, e.g. 'connect_email_mismatch'
